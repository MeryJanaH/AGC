    ...
    <div class="mdk-drawer-layout js-mdk-drawer-layout" data-push data-responsive-width="992px" data-fullbleed>
        <div class="mdk-drawer-layout__content">
            ...
            <div class="mdk-header-layout js-mdk-header-layout" data-has-scrolling-region>
             ...
             {
               require 'LBD.php';
               $sql="SELECT ProjetName,IFNULL(Janv,0) AS Janv
                FROM (SELECT Code_pj, ProjetName FROM Projets) t1
                                      LEFT JOIN
                     (SELECT Code_pj, Visite,COUNT(Visite)as Janv , date_tdebut FROM Calendrier WHERE YEAR(date_tdebut) = '2020' AND MONTH(date_tdebut)='07' AND DAY(date_tdebut) >= '01' AND DAY(date_tdebut) <= '31' AND Visite = 'Chantier' GROUP BY Code_pj) t2
                      ...
                      <div style="width: 100%">
                    	...
                    	<script>
                          ...
                          for ($x = 1; $x <= $d2['count_pj']; $x++) {
                            if($x != $d2['count_pj']){ ?>
                            {type: 'bar', label:'<?php echo $dn['ProjetName']; ?>', data:[<?php echo $dn[1]. "," .$dn[1]. "," .$dn[1]. "," .$dn[1]. "," .$dn[1];
                                                                                       ?>], backgroundColor:'<?php echo $colors[$n++]; ?>',borderColor: 'white',
                    				borderWidth: 1}, <?php
                            }else { ?>
                              {type: 'bar', label:'<?php echo $dn[0]; ?>', data:[<?php echo $dn[1]. "," .$dn[1]. "," .$dn[1]. "," .$dn[1]. "," .$dn[1];
                                                                                         ?>], backgroundColor:'<?php echo $colors[$n++]; ?>',borderColor: 'white',
                      				borderWidth: 1} <?php
                            }
                          } ?>
